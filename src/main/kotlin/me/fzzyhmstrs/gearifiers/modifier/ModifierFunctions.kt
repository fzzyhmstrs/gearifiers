...
object ModifierFunctions {
    ...
    val VORPAL_ATTACK_FUNCTION: EquipmentModifier.DamageFunction =
        EquipmentModifier.DamageFunction { _, user, _, _, amount ->
            if (user.world.random.nextFloat() < GearifiersConfig.getInstance().chances.vorpalChance){
                amount * 10f
            } else {
                amount
            }
    ...
    val INDOMITABLE_DAMAGE_FUNCTION: EquipmentModifier.DamageFunction =
        EquipmentModifier.DamageFunction { _, user, _, _, amount ->
            if (user.world.random.nextFloat() < GearifiersConfig.getInstance().chances.indomitableChance){
    ...
    val SHIELDING_DAMAGE_FUNCTION: EquipmentModifier.DamageFunction =
        EquipmentModifier.DamageFunction { _, user, _, _, amount ->
            if (user.world.random.nextFloat() < GearifiersConfig.getInstance().chances.shieldingChance){
